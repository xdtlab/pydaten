...
class Blockchain(object):
    ...
    def is_next_block(self, block):
        ...
        if fee_transaction.source != config.NOWHERE_NAME or fee_transaction.amount != fees:
            raise InvalidFeeTransaction()

        # Check reward transaction
        reward_transaction = block.transactions[-1]
        if reward_transaction.source != config.SUPPLY_NAME or reward_transaction.amount != self.calculate_reward():
